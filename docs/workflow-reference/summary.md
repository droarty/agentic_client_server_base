...
```
`systemPrompt` supports `{{path.to.field}}` templating (double braces, no sigil). The AI must respond with JSON containing `type`; extra fields become `$message.*` in the triggered handler. `referenceDocs` inlines markdown files on every call (use sparingly — prefer `tools` for larger material). `historyPath` resolves as a dot-path relative to `{ message, user, state }` (same rules as `{{expr}}`), pointing to an array replayed as multi-turn history — `state.*` is refreshed from the database immediately before every `ai` call, so it's always safe for DB-backed history. `tools` names tools (see registry) the model may call — the engine loops tool-use rounds internally and only emits the model's final JSON once it stops calling tools. `maxTurns` (default `8`) caps that tool-use loop — raise it for steps whose tools get called many times per turn.
