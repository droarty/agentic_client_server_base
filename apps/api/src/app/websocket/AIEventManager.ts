  ...
  private async process(request: ValidateTextMessage, config: AiStepConfig, user?: { id: string; email: string }): Promise<void> {
    ...
    try {
      ...
      const raw = await aiService.complete(config.systemPrompt, messages, env.AI_SERVICE_TYPE, {
        ...
        onToolError: (toolName, error) => {
          this.deps.logWorkflowStep?.({
            ...
            executionId,
            stepIndex,
            errorMessage: `tool "${toolName}" failed`,
      ...
      try {
        ...
        // The model sometimes prepends/appends explanatory prose around the required JSON
        // object despite instructions not to — recover by extracting the outermost {...} span
        // before giving up, instead of failing on otherwise-valid output.
